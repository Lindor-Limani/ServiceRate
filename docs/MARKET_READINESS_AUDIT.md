...
| Provision | teilweise | Prozent/Fixbetrag werden serverseitig dezimal berechnet und währungsgebunden gespeichert; keine Gebührenversion/Auditbuchung. |
...
- Angebotspreise, gebuchte Unit Prices, Brutto, Plattformgebühr und Providerforderung werden als `BigDecimal`/`NUMERIC(19,2)` gespeichert. Create/Update weisen ungültige Preise ab; Checkout rundet die drei Abrechnungsbeträge zentral HALF_UP und wahrt `Brutto = Gebühr + Providerforderung`. Stunden und weitere Zahlungspfade bleiben teilweise `Double`. Neue Bookings besitzen eine eigene Währung; Stripe und PayPal übernehmen Geldwerte ohne `Double`-Zwischenschritt. Legacy-Buchungen ohne Snapshot werden beim Checkout fail-closed blockiert.
...
* konkreter Nachweis: PayPal-Order-Erzeugung, PayPal-Capture und Stripe-Checkout werden bookinggebunden pessimistisch serialisiert und senden stabile, nach API-Aufruftyp getrennte Provider-Idempotency-Keys; vollständig persistierte Zustände werden bei Replay ohne neuen Provideraufruf zurückgegeben. PayPal-Capture bindet Order/Booking, Betrag, Währung und Payee fail-closed. Stripe Completed bindet Session/Intent, Betrag, Währung, Application Fee und Destination fail-closed. Zehnfach-Races und echte HTTP-Adaptertests sind für diese Pfade grün. Offen bleiben insbesondere Ledger/Outbox, allgemeine Payment-ID-Constraints, weitere Eventtypen und Recovery nach Parameteränderungen.
...
* Beschreibung: Angebotspreis, Buchungs-Unit-Price, Brutto, Plattformgebühr und Providerforderung sind währungsgebundene Dezimalwerte; Iststunden und weitere Zahlungspfade sind jedoch weiterhin teilweise `Double`, und Steuer-/Gebührenversion sowie vollständiger Vertragssnapshot fehlen.
* konkreter Nachweis: Service Create/Update und Buchungssnapshot verwenden validierte `BigDecimal`-Werte. Checkout berechnet Brutto, Prozent-/Festgebühr und Providerforderung ausschließlich dezimal, rundet HALF_UP auf zwei Stellen, begrenzt die Gebühr auf Brutto und übergibt PayPal/Stripe keine binären Geldwerte. Service-, H2-Persistenz-, Rundungs-/Invarianten-, Provider-Grenzwert-, Adapter- und Parallelitätsregressionen sind grün. Die Gebührenberechnung liest weiterhin eine unversionierte Laufzeitkonfiguration.
* realistisches Angriffs- oder Fehlerszenario: Eine Gebührenkonfiguration wird zwischen Buchung und Checkout geändert; extreme oder parallele Stundenwerte erzeugen abweichende Provisionen; Rundung/Infinity kann in verbliebenen `Double`-Stunden- oder Offlinepfaden weiterhin Daten korrumpieren.
