...
Die synchronen Replay-/Parallelitätsfehler des PayPal-Capture-Pfads und der Stripe-Webhook-Event-ID sind damit geschlossen. SR-F009 bleibt High und Launch-Blocker: Der aktuelle PayPal-Adapter liefert für den Capture-Abgleich weder Betrag, Währung, Booking-ID noch Payee. Stripe prüft gespeicherte Session-/Payment-ID, Betrag, Währung, Payee und erwarteten Ausgangszustand weiterhin nicht vollständig; out-of-order Events können terminale Zustände überschreiben. Das Absturzfenster zwischen nichttransaktionalem Mailversand beziehungsweise erfolgreichem PSP-Effekt und DB-Commit bleibt ohne Provider-Idempotency-Key, Outbox und Recovery offen. Ledger/Reconciliation, versionierte Migration, PostgreSQL-Verifikation und Sandboxnachweis fehlen weiterhin.
