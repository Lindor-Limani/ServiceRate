...
**Status: PARTIALLY COMPLETED.** Der aktive `POST /api/bookings/{id}/checkout`-Pfad prüft nach Customer-Rolle und Booking-Ownership, aber vor Request-Auswertung, Betragsberechnung und jeder Payment-Mutation nun den fachlichen Ausgangszustand. Ausschließlich exakt `ACCEPTED` darf fortfahren; `PENDING`, `REJECTED`, `COMPLETED`, `CANCELLED`, fehlende, leere und unbekannte Werte liefern über die zentrale `ConflictException` HTTP 409. Zusätzlich lädt `PUT /api/bookings/{id}/status` die Buchung mit pessimistischem Zeilen-Lock und erzwingt die Provider-Transitionstabelle `PENDING → ACCEPTED|REJECTED` und `ACCEPTED → COMPLETED`. Alle übrigen gültigen Statuspaare, einschließlich Replay, liefern ohne Mutation 409; ungültige Zielwerte bleiben 400. Service- und HTTP-Regressionen decken die vollständige 5×5-Matrix, Rollen, Ownership und Eingaben ab. Zehn gleichzeitig freigegebene Annahme-/Ablehnungsrequests ergeben genau eine Statusänderung und eine Statusmail.

Die synchronen API-Teilpfade „bereits offene oder abgelehnte Buchung startet Checkout“ und „Provider überspringt oder wiederholt einen fachlichen Statuswechsel“ sind damit geschlossen; konkurrierende Provider-Entscheidungen werden serialisiert. Das Gesamtfinding bleibt High und Launch-Blocker: Checkout und Provider-Statuspfad sind noch nicht durch eine gemeinsame Lock-/Compare-and-Set-Policy gekoppelt. Capture, Offline-Zahlungsverbuchung, Work und Delivery besitzen ebenfalls keine vollständige Transition Policy; Akteur-/Grund-/Zeithistorie, DB-Constraints sowie eine übergreifende Parallelitäts- und Recovery-Suite fehlen.
