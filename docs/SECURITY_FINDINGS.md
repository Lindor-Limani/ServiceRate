...
**Status: PARTIALLY COMPLETED.** Der aktive `POST /api/bookings/{id}/checkout`-Pfad prüft nach Customer-Rolle und Booking-Ownership, aber vor Request-Auswertung, Betragsberechnung und jeder Payment-Mutation nun den fachlichen Ausgangszustand. Ausschließlich exakt `ACCEPTED` darf fortfahren; `PENDING`, `REJECTED`, `COMPLETED`, `CANCELLED`, fehlende, leere und unbekannte Werte liefern über die zentrale `ConflictException` HTTP 409. Service- und HTTP-Regressionen belegen erfolgreiche Offline-/PayPal-Checkouts aus `ACCEPTED`, die vollständige Negativmatrix einschließlich Replay, ausbleibende PayPal-/Stripe-/Repository-Aufrufe und unveränderte Payment-, Betrags- und Checkout-Felder.

Der synchrone API-Teilpfad „bereits offene oder abgelehnte Buchung startet Checkout“ ist damit geschlossen. Das Gesamtfinding bleibt High und Launch-Blocker: Die Statusprüfung und konkurrierende Statusänderungen sind noch nicht durch atomare Compare-and-Set-Updates beziehungsweise eine gemeinsame Lock-Policy gekoppelt. Provider-Statuswechsel, Capture, Offline-Zahlungsverbuchung, Work und Delivery sind ebenfalls noch nicht vollständig durch eine Transition Policy geschützt; Akteur-/Grund-/Zeithistorie, DB-Constraints sowie Parallelitäts- und Recovery-Tests fehlen.
