...
**Status: PARTIALLY COMPLETED.** Der allgemeine Profilpfad `PUT /api/users/{id}` lehnt `paypalMerchantId` und `paypalEmail` vor jeder Entitätsänderung explizit ab. Zusätzlich wurden der clientvertrauende Controller- und Servicepfad `POST /api/providers/me/paypal/onboarding-return` sowie sein Request-DTO entfernt; `SecurityConfig` sperrt die frühere Route unabhängig davon explizit. Das Provider-Frontend bietet keine manuelle Bestätigung mehr an und sendet Rückkehrparameter nicht an das Backend. Eine PayPal-Rückkehr löst stattdessen ausschließlich den vorhandenen serverseitigen Statusabruf aus, dessen Ergebnis erst danach persistiert wird.

Service-, HTTP- und Playwright-Regressionen decken die serververifizierte Statusübernahme, anonymen Zugriff, Customer, Provider, Admin, Replay, persistente Unverändertheit des gesperrten Pfads sowie die sichere Verarbeitung manipulierter Rückkehrparameter auf Desktop und Mobile ab. Der direkte Clienteinfluss auf Payee- und Connected-Werte ist damit in den beiden bestätigten Pfaden beseitigt. Das Finding bleibt Critical und Launch-Blocker, weil der verbleibende Identity-/Partner-Flow noch keinen serverseitig gespeicherten kryptographischen State/Nonce mit Einmalverwendung nachweist und Sellerstatus, Änderungs-Audit, Checkout-Payee-Abgleich sowie Sandbox-/Produktionsabnahme nicht vollständig verifiziert sind.
