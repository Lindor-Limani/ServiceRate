...
**Status: PARTIALLY COMPLETED.** `ServiceOffering.price`, Create-/Update-DTOs und Preisfilter verwenden `BigDecimal`; `createBooking` friert den Wert ohne `Double`-Zwischenschritt mit ISO-Währung ein. Nun verwenden auch `Booking.grossAmount`, `platformFeeAmount` und `providerReceivableAmount`, ihre DTOs und das Schema `BigDecimal`/`NUMERIC(19,2)`. Checkout berechnet Prozent-/Festgebühr dezimal, rundet HALF_UP, deckelt sie auf Brutto und erzwingt `Brutto = Gebühr + Providerforderung`. PayPal und Stripe erhalten diese Werte ohne binäre Geldkonvertierung und weisen korrupte Provider-Sollwerte vor Kommunikation ab.

Service-, HTTP-, Repository-, Security-, lokaler Provider-HTTP- und H2-Zehnfach-Paralleltests belegen Angebotsnormalisierung, exakte Dezimalpersistenz/-suche, Rundungsgrenze, Gebühren-Cap/Summeninvariante, negative Konfiguration ohne Mutation, Provider-Geldwertgrenzen, unveränderte Berechnung nach Angebotsänderung und fail-closed Legacyverhalten. Das Finding bleibt High und Launch-Blocker: Iststunden, Offline-Zahlungsbetrag und Admin-Umsatzaggregation sind teilweise weiterhin `Double`; Gebührenregel, Steuer, Preisart/Einheit und Rechnung sind nicht versioniert beziehungsweise vollständig gesnapshottet. Die Typkonvertierung ist nur im Startup-DDL und nicht als versionierte Migration abgesichert; PostgreSQL-/Sandboxnachweis und Finance-Abnahme fehlen.
