...
**Status: PARTIALLY COMPLETED.** `createBooking` validiert den serverseitig geladenen Angebotspreis auf positiv, endlich, höchstens zwei Nachkommastellen und `NUMERIC(19,2)`-Wertebereich und speichert ihn als `bookedUnitPrice`. Die Angebotswährung wird als tatsächlich bekannter ISO-4217-Code normalisiert und in `bookingCurrencyCode` gespeichert. Buchungsantwort und Checkout-Bruttoberechnung lesen ausschließlich den gebuchten Unit Price; PayPal und Stripe übernehmen ausschließlich die Buchungswährung. Die frühere globale Stripe-Währungsquelle ist entfernt. Fehlende oder korrupte Legacy-Snapshots liefern vor jeder Checkout-Finanz- oder Providermutation 409.

Service-, Security-, lokaler Stripe-HTTP- und H2-Zehnfach-Paralleltests belegen Normalisierung, ungültige Quellen, unveränderte Berechnung nach späterer Preis-/Währungsänderung, fail-closed Legacyverhalten und identische Buchungs-/Providerwährung. Das Finding bleibt High und Launch-Blocker: Brutto, Provision, Providerforderung, Iststunden und Angebotspreis sind teilweise weiterhin `Double`; Gebührenregel, Steuer, Preisart/Einheit und Rechnung sind nicht versioniert beziehungsweise vollständig gesnapshottet. Eine historische Rückbefüllung ist ohne belastbare Vertragsdaten nicht möglich; versionierte Migration, PostgreSQL-/Sandboxnachweis und Finance-Abnahme fehlen.
