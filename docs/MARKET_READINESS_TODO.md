...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (Checkout-Guard für angenommene Buchungen)
- **Geänderte Komponenten:** `BookingService.createCheckout`, bestehende zentrale `ConflictException`-API-Abbildung sowie Booking-Service- und HTTP-Security-Tests.
- **Hinzugefügte Tests:** Erfolgreiche Offline- und PayPal-Checkouts aus `ACCEPTED`; Service-Negativmatrix für `PENDING`, `REJECTED`, `COMPLETED`, `CANCELLED`, null, leer und unbekannt einschließlich Replay; HTTP-409-Matrix für alle persistierbaren Negativzustände mit Prüfung unveränderter Payment-, Betrags- und Checkout-Felder; bestehende Rollen-, Anonym-, Fremdzugriffs- und Nicht-vorhanden-Matrix bleibt aktiv.
- **Verbleibende Einschränkungen:** `POST /api/bookings/{id}/checkout` ist jetzt fail-closed auf exakt `ACCEPTED` begrenzt und lehnt alle anderen Zustände vor Betragsberechnung, Mutation, Persistenz oder PSP-Aufruf mit 409 ab. Das Gesamtticket bleibt offen: Provider-Transitionen, PayPal-Capture, Offline-`record-payment`, Work, Delivery und weitere Zustandsübergänge besitzen noch keine vollständige Policy; atomare Compare-and-Set-Updates, Historie, Parallelitäts-/Recovery-Garantie und DB-Constraints fehlen weiterhin.
