...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich unveränderlicher Stripe-Application-Fee-Sollwert und dessen Completed-Abgleich; frühere PayPal-, Stripe-Idempotenz-, Finanz- und Providerbindungs-Teilfixes bleiben bestehen)
- **Geänderte Komponenten:** `Booking` und bestehendes Startup-Schema um Stripe-Sollbetrag und Soll-Application-Fee in Minor Units sowie ISO-Währung; vorhandene Stripe-Connected-Account-ID als unveränderlicher Checkout-Sollwert; `BookingService`-Replay-/Stale-Guards; `StripeConnectService.createCheckoutSession`/`markCheckoutCompleted`; Stripe-Adapter-, Booking-, signierte Webhook- und Checkout-Parallelitätstests. Die zuvor dokumentierten PayPal-Sollwerte und Idempotenzkomponenten bleiben unverändert Teil des Ticketfortschritts.
- **Hinzugefügte Tests:** Echter Stripe-Checkout-Request belegt Minor-Unit-, Application-Fee-, Währungs- und Destination-Snapshot sowie dessen Wiederverwendung trotz geänderter Quellwerte; null/negativer Betrag, negative oder den Betrag überschreitende Fee, ungültige Währung, unvollständiger und fremder Snapshot werden vor dem Provideraufruf abgewiesen. Signierte Webhook-Regressionen prüfen Happy Path, fehlenden Fee-Alt-Snapshot sowie 22 fehlende, leere oder abweichende Session-/PaymentIntent-Werte für Betrag, empfangenen Betrag, Application Fee, Währung, Paymentstatus, Intentstatus und Transferziel ohne Booking-, Inbox- oder Mailwirkung. Bestehende Session-/Intent-Bindung, Signatur, Replay, Failed→Completed/Completed→Failed, Parallelität und Zehnfach-Checkout bleiben abgedeckt.
- **Verbleibende Einschränkungen:** Stripe speichert beim ersten Checkout `stripeExpectedAmountMinor`, `stripeExpectedApplicationFeeMinor`, normalisierte `stripeCurrencyCode` und `stripeConnectedAccountId`; der Request wird ausschließlich daraus erstellt. `checkout.session.completed` setzt `PAID` nur bei `paid`/`succeeded` und exakter Übereinstimmung von Session-Gesamtbetrag/-Währung sowie PaymentIntent-Betrag, `amount_received`, `application_fee_amount`, Währung und Transferziel. Ein nicht expandierter PaymentIntent muss erfolgreich serverseitig bei Stripe geladen werden; Retrieval-Fehler rollen die Eventverarbeitung zurück. Das Gesamtticket bleibt offen: Der Sollwertsatz ist kein vollständiger Angebots-/Steuer-/Gebührenversions-/Rechnungssnapshot; weitere Eventtypen sind nicht vollständig abgeglichen. Allgemeine `Double`-Geldwerte, Währungen mit abweichenden Minor-Unit-Regeln, abgelaufene Checkouts, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen weiterhin.
...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (Unit-Price-/Währungssnapshot, dezimaler Angebotspreis sowie dezimale Checkout-Abrechnungsbeträge)
- **Geänderte Komponenten:** `ServiceOffering.price`, `Booking.bookedUnitPrice`, `grossAmount`, `platformFeeAmount` und `providerReceivableAmount`, zugehörige DTOs und Startup-Schema auf `BigDecimal`/`NUMERIC(19,2)`; Gebührenkonfiguration und Checkoutberechnung ohne `double`; PayPal-/Stripe-Providerübergänge ohne binären Geldwert-Zwischenschritt.
- **Hinzugefügte Tests:** Service-Create/-Update-Grenzen und Dezimalpersistenz; HALF_UP-Rundungsgrenze, Prozent-/Festgebühr, Gebühren-Cap und Summeninvariante; negative Gebührenkonfiguration ohne Finanzmutation; H2-Persistenz der drei Abrechnungsbeträge; PayPal-/Stripe-Grenzen für negative, übergroße, übergenaue und nicht darstellbare Beträge; bestehende Rollen-/Ownership-, Adapter-, Replay- und Zehnfach-Races.
- **Verbleibende Einschränkungen:** Angebotspreis, gebuchter Unit Price, Brutto, Plattformgebühr und Providerforderung sind dezimal; Legacy-Buchungen ohne belastbaren historischen Wert werden sicher blockiert. Das Gesamtticket bleibt offen: Stunden, Offline-Zahlungsbetrag und Admin-Umsatzaggregation sind weiterhin teilweise `Double`; die Gebührenregel wird erst beim Checkout angewandt und ist nicht als versionierter Buchungssnapshot gespeichert. Steuer, Preisart/Einheit und Rechnung fehlen. Die Typkonvertierung läuft über Startup-DDL statt über eine versionierte Migration; PostgreSQL-/PSP-Sandbox- und Finance-Abnahme fehlen.
- **Priorität/Kategorie:** P0 / Financial Correctness
- **Beschreibung:** Die unmittelbaren `Double`-Fehler für Angebotspreis, Buchungs-Unit-Price und Checkout-Abrechnungsbeträge sind behoben; Stunden, weitere Zahlungspfade und der unvollständige Vertrags-/Gebührensnapshot erlauben weiterhin Rundungs- und Revisionsfehler.
...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich idempotente und parallel serialisierte PayPal-/Stripe-Checkouts)
- **Geänderte Komponenten:** `BookingService.createCheckout`, `BookingService.updateBookingStatus`, `PayPalService.createOrder`, `StripeConnectService.createCheckoutSession`, gemeinsame pessimistisch sperrende Zustandsabfrage im `BookingRepository`, zentrale `ConflictException`-API-Abbildung sowie Booking-Service-, Payment-Adapter-, HTTP-Security- und Parallelitätstests.
- **Hinzugefügte Tests:** Erfolgreiche Offline-/PayPal-Checkouts und Stripe-Erstaufruf aus `ACCEPTED`; PayPal-Replay aus `CHECKOUT_CREATED`, Stripe-Replay aus `CHECKOUT_CREATED`/`FAILED`, andere aktive Zahlungsarten und inkonsistente Providerdaten; echte Adaptertests der Provider-Idempotency-Header; je zehn parallele PayPal-/Stripe-Checkout-Requests mit genau einer Order beziehungsweise Session; bestehende Bookingstatus-Negativmatrix, vollständige Provider-Transitionstabelle, Rollen-/Ownership-Matrix, Provider-Status-Race und Checkout-vs.-Completion-Race.
- **Verbleibende Einschränkungen:** `POST /api/bookings/{id}/checkout` ist fail-closed auf exakt `ACCEPTED` begrenzt. Checkout und Provider-Statusupdate verwenden denselben pessimistischen Zeilen-Lock. PayPal- und Stripe-Checkout-Erzeugung sind für committed und parallele Replays genau einmal wirksam; beide Adapter nutzen stabile bookinggebundene Provider-Keys. Provider-Updates erlauben ausschließlich `PENDING → ACCEPTED|REJECTED` sowie `ACCEPTED → COMPLETED`. Das Gesamtticket bleibt offen: Offline-`record-payment`, Work, Delivery und weitere Übergänge sind nicht vollständig abgesichert; abgelaufene PayPal-Orders beziehungsweise Stripe-Sessions besitzen keinen Erneuerungsworkflow. Der Lock bleibt während des externen Checkout-Aufrufs innerhalb der bestehenden Transaktion gehalten; Outbox-/Timeout-/Recovery-Architektur, Akteur-/Grund-/Zeithistorie, DB-Constraints und PostgreSQL-Verifikation fehlen weiterhin.
