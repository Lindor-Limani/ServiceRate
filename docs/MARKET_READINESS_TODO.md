...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich bookinggebundene PayPal-Order-/Capture- und Stripe-Checkout-Idempotenz)
- **Geänderte Komponenten:** `BookingService.createCheckout`/`capturePayPalPayment` und gemeinsame pessimistische Zustandsabfrage, `PayPalService.createOrder`/`captureOrder` sowie `StripeConnectService.createCheckoutSession` mit stabilen, nach API-Aufruftyp getrennten Provider-Idempotency-Keys, Stripe-Webhook-Inbox/Completed-/Failed-Verarbeitung, zentrale 409-Fehlerabbildung sowie Booking-/Payment-Service-, Adapter-, HTTP-Security- und Parallelitätstests.
- **Hinzugefügte Tests:** PayPal-Order-Normalfall mit committed Replay, Konfliktmatrix, echter HTTP-Request mit stabilem Order-Key, fehlende Booking-ID und Zehnfach-Race; für Capture zusätzlich echter HTTP-Retry mit zweimal demselben `PayPal-Request-Id`, Null-/Blank-Identifier vor Provideraufruf, Replay-/Negativmatrix und Zehnfach-Race. Für Stripe: signierte Webhook-Replay-/ID-/Status-/Reihenfolge-/Parallelitätssuite; Checkout-Normalfall mit committed Replay aus `CHECKOUT_CREATED` und `FAILED`; andere aktive Zahlungsart und inkonsistente Stripe-Daten jeweils ohne Provideraufruf; lokaler echter Stripe-HTTP-Test für den stabilen `Idempotency-Key`; fehlende Booking-ID vor Provideraufruf; zehn parallele Checkout-Requests mit derselben Session/URL und genau einem Adapteraufruf.
- **Verbleibende Einschränkungen:** PayPal-Order, PayPal-Capture und Stripe-Checkout verwenden deterministisch `servicerate-order-{bookingId}`, `servicerate-capture-{bookingId}` beziehungsweise `servicerate-checkout-{bookingId}`. Committete Replays werden unter dem Buchungs-Lock lokal beantwortet; zehn parallele Requests erzeugen in den dokumentierten Pfaden genau einen Adaptereffekt. Damit ist auch das Fehlerfenster eines erneuten PayPal-Capture-POST nach unklarem Providerausgang auf Providerseite idempotent adressiert. Abgelaufene oder serverseitig stornierte Orders/Sessions können noch nicht kontrolliert erneuert werden; der erste Stripe-`savePaymentMethod`-Wert gilt für seine Session. Das Gesamtticket bleibt offen: Betrag, Währung, Booking-ID, Payee/Connected Account und Preisgrundlage werden nicht vollständig gegen Providerdaten geprüft. Ohne unveränderlichen Preis-Snapshot können Parameteränderungen bei anderen Provider-Retries weiterhin Konflikte verursachen. Weitere Eventtypen, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen ebenfalls.
