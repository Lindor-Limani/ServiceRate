...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich bookinggebundene PayPal- und Stripe-Checkout-Idempotenz)
- **Geänderte Komponenten:** `BookingService.createCheckout` und gemeinsame pessimistische Zustandsabfrage, `PayPalService.createOrder` und `StripeConnectService.createCheckoutSession` mit stabilen Provider-Idempotency-Keys, PayPal-Capture, Stripe-Webhook-Inbox/Completed-/Failed-Verarbeitung, zentrale 409-Fehlerabbildung sowie Booking-/Payment-Service-, Adapter-, HTTP-Security- und Parallelitätstests.
- **Hinzugefügte Tests:** PayPal-Order-Normalfall mit committed Replay, Konfliktmatrix für aktive beziehungsweise inkonsistente Zahlungsdaten, echter HTTP-Request mit stabilem `PayPal-Request-Id`, fehlende Booking-ID vor Provideraufruf und Zehnfach-Race mit genau einer Order/URL; PayPal-Capture-Replay-/Negativmatrix und Zehnfach-Race. Für Stripe: signierte Webhook-Replay-/ID-/Status-/Reihenfolge-/Parallelitätssuite; Checkout-Normalfall mit committed Replay aus `CHECKOUT_CREATED` und `FAILED`; andere aktive Zahlungsart und inkonsistente Stripe-Daten jeweils ohne Provideraufruf; lokaler echter Stripe-HTTP-Test für den stabilen `Idempotency-Key`; fehlende Booking-ID vor Provideraufruf; zehn parallele Checkout-Requests mit derselben Session/URL und genau einem Adapteraufruf.
- **Verbleibende Einschränkungen:** PayPal- und Stripe-Checkout-Replays werden unter dem Buchungs-Lock lokal aus vollständig persistierten Providerdaten beantwortet; zehn parallele Requests erzeugen je Provider genau einen Adaptereffekt. Ein möglicher Retry nach unklarem Providerausgang verwendet deterministisch `servicerate-order-{bookingId}` beziehungsweise `servicerate-checkout-{bookingId}`. Abgelaufene oder serverseitig stornierte Orders/Sessions können noch nicht kontrolliert erneuert werden; der erste Stripe-`savePaymentMethod`-Wert gilt für seine Session. Der synchrone PayPal-Capture und die Stripe-Webhook-Inbox/Completed-/Failed-Policy bleiben wie dokumentiert abgesichert. Das Gesamtticket bleibt offen: Betrag, Währung, Booking-ID, Payee/Connected Account und Preisgrundlage werden nicht vollständig gegen Providerdaten geprüft. Ohne unveränderlichen Preis-Snapshot kann ein Retry mit zwischenzeitlich geänderten Parametern einen Provider-Idempotency-Mismatch verursachen. Weitere Eventtypen, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen ebenfalls.
