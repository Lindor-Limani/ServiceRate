...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-14 (allgemeiner Profil-Update-Pfad und clientvertrauender Onboarding-Return)
- **Geänderte Komponenten:** `UserService`, `ProviderProfileController`, `ProviderPayPalOnboardingService`, `SecurityConfig`, PayPal-Onboarding-DTO sowie Provider-Profil-Frontend.
- **Hinzugefügte Tests:** Service-Tests für Merchant-ID, PayPal-E-Mail, beide/leere Felder und Request-Wiederholung; HTTP-Test für Self-/Fremdzugriff und persistente Unverändertheit; Regression für normale Profilfelder; zusätzlicher PayPal-Service-Test für ausschließlich providerseitig gelesene Statuswerte, HTTP-Matrix für den entfernten Return-Pfad mit anonymem Zugriff, allen Rollen, Replay und Persistenzprüfung sowie Playwright-Nachweis auf Desktop/Mobile, dass manipulierte Rückkehrparameter nur den serverseitigen Statusabruf auslösen.
- **Verbleibende Einschränkungen:** Der allgemeine `PUT /api/users/{id}`-Mass-Assignment-Pfad und der clientvertrauende `POST /api/providers/me/paypal/onboarding-return` sind geschlossen. PayPal-Rückkehrparameter werden nicht mehr persistiert, sondern lösen nur noch den serverseitigen Statusabruf aus. Das Gesamtticket bleibt offen, bis der verbleibende Identity-/Partner-Flow einen serverseitig gespeicherten, kryptographisch zufälligen und einmaligen State/Nonce verwendet, Seller- und Berechtigungsstatus vollständig bei PayPal verifiziert, Änderungen auditierbar sind und Sandbox-/Produktionsabnahme vorliegt.
