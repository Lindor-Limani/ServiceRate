...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich unveränderliche Stripe-Checkout-Sollwerte und vollständiger Finanz-/Connected-Account-Abgleich für `checkout.session.completed`; frühere PayPal-, Stripe-Idempotenz- und Providerbindungs-Teilfixes bleiben bestehen)
- **Geänderte Komponenten:** `Booking` und bestehendes Startup-Schema um Stripe-Sollbetrag in Minor Units und ISO-Währung; vorhandene Stripe-Connected-Account-ID als unveränderlicher Checkout-Sollwert; `BookingService`-Replay-/Stale-Guards; `StripeConnectService.createCheckoutSession`/`markCheckoutCompleted`; Stripe-Adapter-, Booking-, signierte Webhook- und Checkout-Parallelitätstests. Die zuvor dokumentierten PayPal-Sollwerte und Idempotenzkomponenten bleiben unverändert Teil des Ticketfortschritts.
- **Hinzugefügte Tests:** Echter Stripe-Checkout-Request belegt Minor-Unit-, Währungs- und Destination-Snapshot sowie dessen Wiederverwendung trotz geänderter Quellwerte; null/negativer Betrag, ungültige Währung, unvollständiger und fremder Snapshot werden vor dem Provideraufruf abgewiesen. Signierte Webhook-Regressionen prüfen Happy Path, fehlenden Alt-Snapshot sowie 19 fehlende, leere oder abweichende Session-/PaymentIntent-Werte für Betrag, empfangenen Betrag, Währung, Paymentstatus, Intentstatus und Transferziel ohne Booking-, Inbox- oder Mailwirkung. Bestehende Session-/Intent-Bindung, Signatur, Replay, Failed→Completed/Completed→Failed, Parallelität und Zehnfach-Checkout bleiben abgedeckt.
- **Verbleibende Einschränkungen:** Stripe speichert beim ersten Checkout `stripeExpectedAmountMinor`, normalisierte `stripeCurrencyCode` und `stripeConnectedAccountId`; der Request wird ausschließlich daraus erstellt. `checkout.session.completed` setzt `PAID` nur bei `paid`/`succeeded` und exakter Übereinstimmung von Session-Gesamtbetrag/-Währung sowie PaymentIntent-Betrag, `amount_received`, Währung und Transferziel. Ein nicht expandierter PaymentIntent muss erfolgreich serverseitig bei Stripe geladen werden; Retrieval-Fehler rollen die Eventverarbeitung zurück. Das Gesamtticket bleibt offen: Der Sollwertsatz ist kein vollständiger Angebots-/Steuer-/Gebühren-/Rechnungssnapshot; Application Fee und weitere Eventtypen sind nicht vollständig abgeglichen. Allgemeine `Double`-Geldwerte, Währungen mit abweichenden Minor-Unit-Regeln, abgelaufene Checkouts, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen weiterhin.
