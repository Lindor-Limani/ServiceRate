...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich unveränderliche PayPal-Capture-Sollwerte und Betrag-/Währungs-/Payee-Abgleich; frühere Idempotenz- und Providerbindungs-Teilfixes bleiben bestehen)
- **Geänderte Komponenten:** `Booking` und bestehendes Startup-Schema um PayPal-Sollbetrag, ISO-Währung und Payee-Merchant-ID; `BookingService.createCheckout`/`capturePayPalPayment`; `PayPalService.createOrder`/`captureOrder`; PayPal-Service-, Adapter-, HTTP-Security-, Checkout- und Capture-Parallelitätstests. Frühere Stripe-Webhook-/Checkout- und PayPal-Idempotenzkomponenten bleiben unverändert Teil des Ticketfortschritts.
- **Hinzugefügte Tests:** PayPal-Order-Request verwendet gespeicherte Sollwerte auch bei abweichenden aktuellen Angebotswerten; fehlender oder fremder Adapter-Snapshot wird vor HTTP abgewiesen; Capture-HTTP-Retry extrahiert zusätzlich Betrag, Währung und Payee; Service-Matrix für fehlenden/falschen Betrag, fehlende/leere/falsche Währung und fehlenden/leeren/fremden Payee ohne Mutation; fehlender Alt-Snapshot ohne Provideraufruf; Happy Path und committed Replay; bestehende Rollen-/Ownership-Matrix; Zehnfach-Checkout persistiert genau einen Sollwertsatz und Zehnfach-Capture erzeugt genau einen Provider-/Persistenz-/Mail-Effekt.
- **Verbleibende Einschränkungen:** Der PayPal-Teilpfad speichert beim ersten Checkout `paypalExpectedAmount` mit fester Zweier-Skala, normalisierte `paypalCurrencyCode` und `paypalPayeeMerchantId`; die Order wird ausschließlich daraus erstellt. Capture setzt `PAID` erst nach exakter Übereinstimmung dieser Werte sowie Order-ID, `reference_id` und `custom_id`. Unvollständige bestehende Checkouts werden fail-closed nicht wiederverwendet oder gecaptured. Das Gesamtticket bleibt offen: Stripe prüft Betrag, Währung und Connected Account noch nicht vollständig; der PayPal-Snapshot ist kein vollständiger Angebots-/Steuer-/Gebühren-/Rechnungssnapshot. Allgemeine `Double`-Geldwerte, weitere Eventtypen, abgelaufene Checkouts, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen weiterhin.
