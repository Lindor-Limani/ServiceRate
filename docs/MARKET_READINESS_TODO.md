...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich PayPal-Capture-Providerbindung sowie bookinggebundene PayPal-Order-/Capture- und Stripe-Checkout-Idempotenz)
- **Geänderte Komponenten:** `BookingService.createCheckout`/`capturePayPalPayment` und gemeinsame pessimistische Zustandsabfrage, `PayPalService.createOrder`/`captureOrder` einschließlich vollständiger Capture-Repräsentation sowie `StripeConnectService.createCheckoutSession` mit stabilen, nach API-Aufruftyp getrennten Provider-Idempotency-Keys, Stripe-Webhook-Inbox/Completed-/Failed-Verarbeitung, zentrale 409-Fehlerabbildung sowie Booking-/Payment-Service-, Adapter-, HTTP-Security- und Parallelitätstests.
- **Hinzugefügte Tests:** PayPal-Order-Normalfall mit committed Replay, Konfliktmatrix, echter HTTP-Request mit stabilem Order-Key, fehlende Booking-ID und Zehnfach-Race; für Capture zusätzlich echter HTTP-Retry mit identischem `PayPal-Request-Id`, `Prefer: return=representation`, Extraktion von Order-/Reference-/Custom-ID, Null-/Blank-Identifier, neun fehlende, leere oder fremde Providerzuordnungen ohne Mutation, Replay-/Status-/Rollen-/Ownership-Matrix und Zehnfach-Race. Für Stripe: signierte Webhook-Replay-/ID-/Status-/Reihenfolge-/Parallelitätssuite; Checkout-Normalfall mit committed Replay aus `CHECKOUT_CREATED` und `FAILED`; andere aktive Zahlungsart und inkonsistente Stripe-Daten jeweils ohne Provideraufruf; lokaler echter Stripe-HTTP-Test für den stabilen `Idempotency-Key`; fehlende Booking-ID vor Provideraufruf; zehn parallele Checkout-Requests mit derselben Session/URL und genau einem Adapteraufruf.
- **Verbleibende Einschränkungen:** PayPal-Order, PayPal-Capture und Stripe-Checkout verwenden deterministisch `servicerate-order-{bookingId}`, `servicerate-capture-{bookingId}` beziehungsweise `servicerate-checkout-{bookingId}`. PayPal-Capture setzt `PAID` nur nach exakter Übereinstimmung von Provider-Order-ID, `reference_id` und `custom_id` mit der gespeicherten Order und Buchung. Committete Replays werden unter dem Buchungs-Lock lokal beantwortet; zehn parallele Requests erzeugen in den dokumentierten Pfaden genau einen Adaptereffekt. Abgelaufene oder serverseitig stornierte Orders/Sessions können noch nicht kontrolliert erneuert werden; der erste Stripe-`savePaymentMethod`-Wert gilt für seine Session. Das Gesamtticket bleibt offen: Betrag, Währung, Payee/Connected Account und Preisgrundlage werden nicht vollständig gegen Providerdaten geprüft. Ohne unveränderlichen Preis-/Währungs-/Payee-Snapshot können Parameteränderungen und spätere Stammdatenänderungen weiterhin Konflikte oder ungeeignete Vergleichsgrundlagen verursachen. Weitere Eventtypen, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen ebenfalls.
