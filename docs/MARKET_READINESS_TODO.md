...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich bookinggebundene Stripe-Checkout-Idempotenz)
- **Geänderte Komponenten:** `BookingService.createCheckout` und gemeinsame pessimistische Zustandsabfrage, `StripeConnectService.createCheckoutSession` mit stabilem Provider-Idempotency-Key, PayPal-Capture, Stripe-Webhook-Inbox/Completed-/Failed-Verarbeitung, zentrale 409-Fehlerabbildung sowie Booking-/Stripe-Service-, Adapter-, HTTP-Security- und Parallelitätstests.
- **Hinzugefügte Tests:** PayPal-Capture-Replay-/Negativmatrix und Zehnfach-Race. Für Stripe: signierte Webhook-Replay-/ID-/Status-/Reihenfolge-/Parallelitätssuite; Checkout-Normalfall mit committed Replay aus `CHECKOUT_CREATED` und `FAILED`; andere aktive Zahlungsart und inkonsistente Stripe-Daten jeweils ohne Provideraufruf; lokaler echter Stripe-HTTP-Test für den stabilen `Idempotency-Key`; fehlende Booking-ID vor Provideraufruf; zehn parallele Checkout-Requests mit derselben Session/URL und genau einem Adapteraufruf.
- **Verbleibende Einschränkungen:** Stripe-Checkout-Replays werden unter dem Buchungs-Lock lokal aus der committed Session beantwortet; zehn parallele Requests erzeugen genau einen Adaptereffekt. Ein möglicher Retry nach unklarem Providerausgang verwendet deterministisch `servicerate-checkout-{bookingId}`, sodass Stripe keine zweite Session erzeugt. Der erste `savePaymentMethod`-Wert gilt für diese Session; eine abgelaufene oder serverseitig stornierte Session kann noch nicht kontrolliert erneuert werden. Der synchrone PayPal-Capture und die Stripe-Webhook-Inbox/Completed-/Failed-Policy bleiben wie dokumentiert abgesichert. Das Gesamtticket bleibt offen: PayPal-Order-Erzeugung besitzt keinen Provider-Idempotency-Key; Betrag, Währung, Booking-ID, Payee/Connected Account und Preisgrundlage werden nicht vollständig gegen Providerdaten geprüft. Ohne unveränderlichen Preis-Snapshot kann ein Retry mit zwischenzeitlich geänderten Parametern von Stripe als Idempotency-Mismatch abgewiesen werden. Weitere Eventtypen, Outbox, Ledger, Reconciliation, versionierte DB-Migration sowie PostgreSQL-/Sandboxverifikation fehlen ebenfalls.
...
- **Status:** PARTIALLY COMPLETED
- **Abschlussdatum:** 2026-07-15 (zusätzlich idempotenter und parallel serialisierter Stripe-Checkout)
- **Geänderte Komponenten:** `BookingService.createCheckout`, `BookingService.updateBookingStatus`, `StripeConnectService.createCheckoutSession`, gemeinsame pessimistisch sperrende Zustandsabfrage im `BookingRepository`, zentrale `ConflictException`-API-Abbildung sowie Booking-Service-, Stripe-Adapter-, HTTP-Security- und Parallelitätstests.
- **Hinzugefügte Tests:** Erfolgreiche Offline-/PayPal-Checkouts und Stripe-Erstaufruf aus `ACCEPTED`; Stripe-Replay aus `CHECKOUT_CREATED`/`FAILED`, andere aktive Zahlungsart und inkonsistente Stripe-Daten; echter Adaptertest des Provider-Idempotency-Headers; zehn parallele Stripe-Checkout-Requests mit genau einer Session; bestehende Bookingstatus-Negativmatrix, vollständige Provider-Transitionstabelle, Rollen-/Ownership-Matrix, Provider-Status-Race und Checkout-vs.-Completion-Race.
- **Verbleibende Einschränkungen:** `POST /api/bookings/{id}/checkout` ist fail-closed auf exakt `ACCEPTED` begrenzt. Checkout und Provider-Statusupdate verwenden denselben pessimistischen Zeilen-Lock. Stripe-Checkout-Erzeugung ist für committed und parallele Replays genau einmal wirksam; der Adapter nutzt zusätzlich einen stabilen bookinggebundenen Stripe-Key für Retries an der Providergrenze. Provider-Updates erlauben ausschließlich `PENDING → ACCEPTED|REJECTED` sowie `ACCEPTED → COMPLETED`. Das Gesamtticket bleibt offen: PayPal-Order-Erzeugung, Offline-`record-payment`, Work, Delivery und weitere Übergänge sind nicht vollständig abgesichert; abgelaufene Stripe-Sessions besitzen keinen Erneuerungsworkflow. Der Lock bleibt während des externen Checkout-Aufrufs innerhalb der bestehenden Transaktion gehalten; Outbox-/Timeout-/Recovery-Architektur, Akteur-/Grund-/Zeithistorie, DB-Constraints und PostgreSQL-Verifikation fehlen weiterhin.
