...
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(Seit 2026-07-15 wird der synchrone PayPal-Capture pessimistisch serialisiert und committed Replays bleiben nebenwirkungsfrei. Stripe-Webhooks beanspruchen die signaturgeprüfte Provider-Event-ID transaktional über einen eindeutigen Inbox-Constraint. Completed/Failed verwenden denselben pessimistischen Buchungs-Lock, verlangen `CARD` sowie die gespeicherte Session-/Payment-Intent-Zuordnung und behandeln `PAID` als terminal; sequentielle und parallele Completed-/Failed-Reihenfolgen enden deterministisch in `PAID`. Service-/HTTP-Negativmatrizen einschließlich echter Stripe-HMAC-Signatur sind grün. Das Gate bleibt wegen fehlendem Betrag-/Währungs-/Booking-/Payee-/Connected-Account-Abgleich, weiteren Eventlebenszyklen, Crash-Fenstern externer Effekte, Checkout-/Provider-Idempotency-Key, Outbox, Ledger, Preis-Snapshot, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
