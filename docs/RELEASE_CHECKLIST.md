...
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(PayPal und Stripe speichern beim Checkout providergebundene Betrags-/Währungs-/Empfänger-Sollwerte und erstellen ihre Requests daraus. PayPal-Capture gleicht Order/Booking, Betrag, Währung und Payee ab. Stripe `checkout.session.completed` verlangt `paid`/`succeeded` und gleicht Session, PaymentIntent, Betrag, `amount_received`, Währung und Connected Account ab; fehlende oder abweichende Werte rollen Inbox und Bookingwirkung zurück. HTTP-, Adapter-, Replay-, Reihenfolge- und Parallelitätsregressionen sind grün. Das Gate bleibt wegen vollständigem Preis-/Gebühren-/Steuer-Snapshot, Application-Fee-/weiteren Eventabgleichen, allgemeinen `Double`-Geldwerten und Multi-Währungsregeln, abgelaufener Checkout-Erneuerung, Outbox, Ledger, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
