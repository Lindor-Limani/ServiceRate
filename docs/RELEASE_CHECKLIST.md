...
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(PayPal-Order-/Capture-Replays, Stripe-Checkout-Erzeugung und Stripe-Webhooks sind in den dokumentierten Pfaden serialisiert und idempotent. PayPal speichert beim Checkout erwarteten Betrag, ISO-Währung und Payee-Merchant-ID, erstellt die Order ausschließlich daraus und gleicht diese Werte sowie Order-ID, `reference_id` und `custom_id` vor `PAID` mit der vollständigen Capture-Repräsentation ab. Fehlende oder abweichende Werte bleiben ohne Paymentmutation; HTTP- und Zehnfach-Race-Regressionen sind grün. Das Gate bleibt wegen Stripe-Betrag-/Währungs-/Connected-Account-Abgleich, vollständigem Preis-/Gebühren-/Steuer-Snapshot, allgemeinen `Double`-Geldwerten, abgelaufener Checkout-Erneuerung, weiteren Eventlebenszyklen, Outbox, Ledger, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
