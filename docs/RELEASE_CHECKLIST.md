...
- [ ] **SR-F008/SR-F011:** Buchungszustandsmaschine und Slotreservierung sind atomar; Race-Suite bestanden. *(Seit 2026-07-15 startet Checkout ausschließlich aus `ACCEPTED`; Checkout und Provider-Statusupdate verwenden denselben pessimistischen Lock, und Provider-Updates folgen der getesteten Transitionstabelle. PayPal-Replays aus `CHECKOUT_CREATED` sowie Stripe-Replays aus `CHECKOUT_CREATED`/`FAILED` geben dieselbe Order beziehungsweise Session ohne Adapteraufruf zurück; andere aktive Zahlungen oder inkonsistente Providerdaten liefern 409. Je ein Zehnfach-Race erzeugt genau eine PayPal-Order beziehungsweise Stripe-Session. Das Gate bleibt wegen Offline-Idempotenz, abgelaufener Provider-Checkouts, übriger Transitionen, Historie, DB-Constraints/PostgreSQL-Nachweis, Slotreservierung sowie der übergreifenden Recovery-Suite offen.)*
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(PayPal und Stripe speichern beim Checkout providergebundene Betrags-/Währungs-/Empfänger-Sollwerte und erstellen ihre Requests daraus; Stripe friert zusätzlich die Application Fee ein. PayPal-Capture gleicht Order/Booking, Betrag, Währung und Payee ab. Stripe `checkout.session.completed` verlangt `paid`/`succeeded` und gleicht Session, PaymentIntent, Betrag, `amount_received`, `application_fee_amount`, Währung und Connected Account ab; fehlende oder abweichende Werte rollen Inbox und Bookingwirkung zurück. HTTP-, Adapter-, Replay-, Reihenfolge- und Parallelitätsregressionen sind grün. Das Gate bleibt wegen vollständigem Preis-/Gebührenversions-/Steuer-Snapshot, weiteren Eventabgleichen, allgemeinen `Double`-Geldwerten und Multi-Währungsregeln, abgelaufener Checkout-Erneuerung, Outbox, Ledger, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
