...
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(Angebotspreise werden bei Create/Update zentral auf positive `BigDecimal`-/`NUMERIC(19,2)`-Werte begrenzt; neue Buchungen frieren diesen Unit Price und die ISO-Währung ein. Anzeige, Bruttoberechnung, PayPal und Stripe verwenden diese Werte trotz späterer Angebotsänderung. Service-/HTTP-Grenzwert-, Persistenz-/Such-, Adapter-, Replay-, Reihenfolge- und Parallelitätsregressionen sind grün. Das Gate bleibt wegen vollständigem Gebührenversions-/Steuer-/Rechnungssnapshot, verbliebenen `Double`-Geldwerten, vollständigen Multi-Währungsregeln, versionierter Migration/Legacy-Entscheidung, weiteren Eventabgleichen, abgelaufener Checkout-Erneuerung, Outbox, Ledger, Reconciliation sowie PostgreSQL-/Sandboxnachweis offen.)*
