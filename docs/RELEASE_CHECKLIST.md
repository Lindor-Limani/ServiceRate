...
- [ ] **SR-F008/SR-F011:** Buchungszustandsmaschine und Slotreservierung sind atomar; Race-Suite bestanden. *(Seit 2026-07-15 startet Checkout ausschließlich aus `ACCEPTED`; Checkout und Provider-Statusupdate verwenden denselben pessimistischen Lock, und Provider-Updates folgen der getesteten Transitionstabelle. Stripe-Checkout-Replays aus `CHECKOUT_CREATED`/`FAILED` geben dieselbe Session ohne Adapteraufruf zurück; andere aktive Zahlungen oder inkonsistente Stripe-Daten liefern 409. Ein Zehnfach-Race erzeugt genau eine Stripe-Session. Das Gate bleibt wegen PayPal-Order-/Offline-Idempotenz, abgelaufener Stripe-Sessions, übriger Transitionen, Historie, DB-Constraints/PostgreSQL-Nachweis, Slotreservierung sowie der übergreifenden Recovery-Suite offen.)*
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(PayPal-Capture-Replays, Stripe-Checkout-Erzeugung und Stripe-Webhooks sind in den dokumentierten Pfaden serialisiert und idempotent. Stripe-Checkout nutzt zusätzlich einen echten getesteten bookinggebundenen Provider-Idempotency-Key; ein Zehnfach-Race erzeugt genau einen Adaptereffekt. Completed/Failed prüfen Provider-/Session-/Payment-Intent-Zuordnung und enden reihenfolgeunabhängig in `PAID`. Das Gate bleibt wegen PayPal-Order-Idempotenz, Betrag-/Währungs-/Booking-/Payee-/Connected-Account-Abgleich, Preis-Snapshot, weiteren Eventlebenszyklen, Outbox, Ledger, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
