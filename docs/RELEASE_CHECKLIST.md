...
- [ ] **SR-F004:** PayPal-Merchant-Zuordnung ausschließlich serverseitig verifiziert; State/Nonce und Änderungs-Audit grün. *(Direkte Profiländerung und clientvertrauende Legacy-Returns wurden am 2026-07-14 geschlossen. Seit 2026-07-15 bindet ein gehashter, 15 Minuten gültiger und pessimistisch genau einmal konsumierter 256-Bit-State den aktiven Partner-Return an den authentifizierten Provider; Rollen-, Ablauf-, Fremdzugriffs-, Replay-, Zehnfach-Parallelitäts- und Browsertests sind grün. Der Checkout verlangt zentral `CONNECTED`, positive Permission-/E-Mail-Flags und eine Merchant-ID; PayPal erhält ausschließlich diese `merchant_id`, und unvollständige sowie reine E-Mail-Zuordnungen werden vor dem PSP-Aufruf abgewiesen. Das Gate bleibt wegen vollständiger Seller-/Berechtigungsabnahme einschließlich Entzugserkennung, Änderungs-Audit, versionierter PostgreSQL-Migration und PayPal-Sandbox-/Produktionsnachweis offen.)*
...
- [ ] **SR-F006:** Reviews ausschließlich durch den authentifizierten Customer der abgeschlossenen Buchung und genau einmal erzeugbar. *(Rollen-/Ownership-Prüfung, HTTP-409-Replay, pessimistische Serialisierung, eindeutiger DB-Index und Zehnfach-Paralleltest am 2026-07-14 umgesetzt; Release-Gate bleibt bis zur PostgreSQL-/Migrationsverifikation unmarkiert.)*
...
- [ ] **SR-F008/SR-F011:** Buchungszustandsmaschine und Slotreservierung sind atomar; Race-Suite bestanden. *(Seit 2026-07-15 startet `POST /api/bookings/{id}/checkout` ausschließlich aus `ACCEPTED`; alle anderen, fehlenden oder unbekannten Zustände liefern vor Payment-Seiteneffekten 409. Checkout und Provider-Statusupdate verwenden denselben pessimistischen Buchungs-Lock. Provider-Updates erlauben ausschließlich `PENDING → ACCEPTED|REJECTED` sowie `ACCEPTED → COMPLETED`; eine vollständige 5×5-Service-Matrix, HTTP-Regressionen für Rollen, Ownership, Eingaben und Replay, ein Zehnfach-Provider-Race sowie ein gesteuerter Checkout-vs.-Completion-Race ohne Lost Update sind grün. Das Gate bleibt wegen Checkout-/PSP-Idempotenz, Capture und übriger Transitionen, Historie, DB-Constraints/PostgreSQL-Nachweis, Slotreservierung sowie der übergreifenden Race-/Recovery-Suite offen.)*
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(Seit 2026-07-15 wird der synchrone PayPal-Capture pessimistisch serialisiert, verlangt `PAYPAL`, exakte Order und `CHECKOUT_CREATED`, akzeptiert nur `COMPLETED` mit Capture-ID und behandelt committed Replays nebenwirkungsfrei. Stripe-Webhooks beanspruchen eine signaturgeprüfte Provider-Event-ID transaktional über einen eindeutigen Inbox-Constraint; committed sequentielle und zehnfach parallele Replays erzeugen genau einen Booking-/Mail-Effekt, Fachfehler rollen den Inbox-Eintrag für sichere Retries zurück. Service-/HTTP-Negativmatrizen einschließlich echter Stripe-HMAC-Signatur sind grün. Das Gate bleibt wegen fehlendem Betrag-/Währungs-/Booking-/Payee-/Session-Abgleich, out-of-order Verarbeitung, Crash-Fenstern externer Effekte, Checkout-/Provider-Idempotency-Key, Outbox, Ledger, Preis-Snapshot, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
- [ ] **SR-F015:** finale SCA ohne anwendbare Critical/High-Befunde; SBOM archiviert. *(Framework-Runtime am 2026-07-14 aktualisiert und per Gradle-Gate fixiert; eine kanonische CycloneDX-1.6-Runtime-SBOM wird reproduzierbar erzeugt und in `check` validiert. Kontinuierliches CI-SCA sowie Scan und unveränderliche Archivierung der SBOM des finalen Artefakts bleiben offen.)*
