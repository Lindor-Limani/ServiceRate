...
- [ ] **SR-F009/SR-F010/SR-F017:** Ledger, Idempotenz, Währung, Preis-Snapshot und Reconciliation fachlich/technisch abgenommen. *(PayPal-Order-/Capture-Replays, Stripe-Checkout-Erzeugung und Stripe-Webhooks sind in den dokumentierten Pfaden serialisiert und idempotent. PayPal-Order, PayPal-Capture und Stripe-Checkout nutzen echte getestete, bookinggebundene und nach API-Aufruftyp getrennte Provider-Idempotency-Keys; PayPal-Capture fordert die vollständige Providerrepräsentation an und prüft Order-ID, `reference_id` und `custom_id` gegen die gelockte Buchung. Ein HTTP-Retry belegt Header und Identitätsextraktion, und die Zehnfach-Races erzeugen je genau einen Adaptereffekt. Das Gate bleibt wegen Betrag-/Währungs-/Payee-/Connected-Account-Abgleich, Preis-/Währungs-/Payee-Snapshot, abgelaufener Checkout-Erneuerung, weiteren Eventlebenszyklen, Outbox, Ledger, Reconciliation, versionierter Migration sowie PostgreSQL-/Sandboxnachweis offen.)*
