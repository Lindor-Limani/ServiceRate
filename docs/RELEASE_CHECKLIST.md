...
- [ ] **SR-F008/SR-F011:** Buchungszustandsmaschine und Slotreservierung sind atomar; Race-Suite bestanden. *(Seit 2026-07-15 startet `POST /api/bookings/{id}/checkout` ausschließlich aus `ACCEPTED`; alle anderen, fehlenden oder unbekannten Zustände liefern vor Payment-Seiteneffekten 409. Provider-Statusupdates werden pessimistisch serialisiert und erlauben ausschließlich `PENDING → ACCEPTED|REJECTED` sowie `ACCEPTED → COMPLETED`; eine vollständige 5×5-Service-Matrix, HTTP-Regressionen für Rollen, Ownership, Eingaben und Replay sowie ein Zehnfach-Race mit genau einer Änderung und Statusmail sind grün. Das Gate bleibt wegen der fehlenden gemeinsamen Lock-Policy mit Checkout/Payment, der übrigen Transitionen, Historie, DB-Constraints, Slotreservierung sowie der übergreifenden Race-/Recovery-Suite offen.)*
