...
- [ ] **SR-F008/SR-F011:** Buchungszustandsmaschine und Slotreservierung sind atomar; Race-Suite bestanden. *(Seit 2026-07-15 startet `POST /api/bookings/{id}/checkout` ausschließlich aus `ACCEPTED`; alle anderen, fehlenden oder unbekannten Zustände liefern vor Payment-Seiteneffekten 409 und sind per Service-/HTTP-Matrix einschließlich Replay regressionsgetestet. Das Gate bleibt wegen der übrigen Transitionen, atomarer Updates, Historie, Slotreservierung und Race-/Recovery-Suite offen.)*
